...
use crate::{AppState, http::common::{api_error::{ApiError, ErrorBody}, middleware::auth::entities::TokenValidator, response::Response, validator::ValidatedJson}};
...
pub async fn signup(
...
) -> Result<(AppendHeaders<[(axum::http::HeaderName, String); 1]>, Response<CreateDriverResponse>), ApiError> {
    ...
    let create_tokens_fn = |driver_id: uuid::Uuid| -> Result<(String, String), DriverError> {
        auth_validator.create_tokens(driver_id)
            .map_err(|_| DriverError::DatabaseError)
    };
    
    let (access_token, refresh_token_cookie) = state.service.generate_tokens(
        driver,
        create_tokens_fn,
    ...
    Ok((AppendHeaders(headers), Response::created(CreateDriverResponse {
