use plannify_driver_api_core::{
    ...
    domain::workday::port::WorkdayDatabaseRepository,
};
...
mod tests {
    ...
    impl StubWorkdayDb {
        fn returns_count(n: u32) -> Self {
            Self { delete_result: Ok(n) }
        }
        fn fails() -> Self {
            Self { delete_result: Err(WorkdayError::DatabaseError) }
        }
    }
    ...
    impl WorkdayDatabaseRepository for StubWorkdayDb {
        ...
        async fn create_workday_document(&self, _: Uuid, _: i32, _: i32, _: String, _: String) -> Result<WorkdayDocument, WorkdayError> { unreachable!() }
        async fn get_pending_document_months(&self, _: NaiveDate) -> Result<Vec<(Uuid, i32, i32)>, WorkdayError> { unreachable!() }
    }
