    ...
    D: DriverRepository,
{
    ...
    async fn create_driver(
        ...
        if email_list_deny.contains(&email_domain.to_string()) {
            error!("Attempt to sign up with denylisted email domain ({}) : {}", email_domain, create_request.email);
        ...
        let password_hash = argon2.hash_password(create_request.password.as_bytes(), &salt)
            .map_err(|e| {
        ...
        F: Fn(uuid::Uuid) -> Result<(String, String), DriverError> + Send + Sync,
    {
        ...
        let params = Params::new(19 * 1024, 2, 1, None)
            .map_err(|e| {
                error!("Failed to create Argon2 params for refresh token hashing: {}", e);
                DriverError::Internal
            })?; // 19 MiB, 2 itérations, 1 thread
        let argon2 = Argon2::new(Algorithm::Argon2id, Version::V0x13, params);

        let token_hash = argon2.hash_password(refresh_token.as_bytes(), &salt)
            .map_err(|e| {
